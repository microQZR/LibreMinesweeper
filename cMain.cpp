...
cMain::cMain() : wxFrame(nullptr, wxID_ANY, "OneLoneCoder.com - wxWidgets!", wxPoint(100, 100), wxSize(600, 665))
{
	...
	for (int x = 0; x < nFieldWidth; x++)
	{
		for (int y =0; y <nFieldHeight; y++)
		{
			...
			btn[y*nFieldWidth + x]->Bind(wxEVT_COMMAND_BUTTON_CLICKED, &cMain::OnButtonClicked, this); //Binding left click events to buttons
	...
	//Set up bitmap buttons for end game dial
	...
	//Fit dialog elements to sizers of end game dial
	...
	vboxEndGame->Add(hboxExtra_EndGame, 1, wxALIGN_CENTER); //add extra hbox
	...
	//Define sizers for use by settings dial
	...
	//Set up static text controls and slider control for settings dial
	...
	//Fit dialog elements to sizers of settings dial
	...
	int xyz = SettingsDial->ShowModal(); //Show welcome-settings dialog
}
